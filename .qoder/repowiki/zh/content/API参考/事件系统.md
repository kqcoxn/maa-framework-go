...
## 更新摘要
**已做更改**
- 更新了项目结构描述，反映事件系统重构，包括新的手写事件适配器和删除的代码生成文件
- 更新了核心组件描述，准确反映当前事件接口和适配器的实现
- 修正了事件接口与适配器章节，反映手写实现而非代码生成
- 更新了事件回调注册与管理机制，反映当前的注册流程
- 移除了关于 `event_sinks_gen.go` 的所有引用，该文件已被删除
...
- 事件接口与适配器：在 event_sinks.go 中通过手写方式实现 Tasker/Resource/Controller/Context 四类事件接口及 Adapter，提供按事件粒度注册的便捷方法。
...
- 事件回调接口
  - TaskerEventSink：包含 OnTaskerTask 回调方法，接收 Tasker 实例指针
  - ResourceEventSink：包含 OnResourceLoading 回调方法，接收 Resource 实例指针
  - ControllerEventSink：包含 OnControllerAction 回调方法，接收 Controller 实例指针
  - ContextEventSink：包含 OnNodePipelineNode、OnNodeRecognitionNode、OnNodeActionNode、OnNodeNextList、OnNodeRecognition、OnNodeAction 六个回调方法，接收 Context 实例指针（且支持在 Tasker 上以 AddContextSink 注册）
...
- TaskerEventSink/ResourceEventSink/ControllerEventSink/ContextEventSink
  - TaskerEventSink 包含 OnTaskerTask 回调方法
  - ResourceEventSink 包含 OnResourceLoading 回调方法
  - ControllerEventSink 包含 OnControllerAction 回调方法
  - ContextEventSink 包含 OnNodePipelineNode、OnNodeRecognitionNode、OnNodeActionNode、OnNodeNextList、OnNodeRecognition、OnNodeAction 六个回调方法
  - 对应 Adapter 提供单事件回调的轻量封装，便于直接注册某个事件的处理函数
- 各资源对象的便捷注册方法
  - Tasker：OnTaskerTask(fn) 返回 sinkId，可用于 RemoveSink
  - Resource：OnResourceLoading(fn) 返回 sinkId，可用于 RemoveSink
  - Controller：OnControllerAction(fn) 返回 sinkId，可用于 RemoveSink
  - Context：通过 Tasker 的 OnNodePipelineNodeInContext(fn)、OnNodeRecognitionNodeInContext(fn)、OnNodeActionNodeInContext(fn)、OnNodeNextListInContext(fn)、OnNodeRecognitionInContext(fn)、OnNodeActionInContext(fn) 注册，回调时携带 Context 实例
...
+OnNodeNextList(ctx, status, detail)
